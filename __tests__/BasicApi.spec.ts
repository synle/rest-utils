...
describe("BasicApi", () => {
  it("Should work for doGet", async () => {
    ...
    expect(resp.config.url).toBe(
      "https://httpbin.org/get?someName1=someValue1&someName2=someValue2"
    );
  ...
  it("Should work for doPost", async () => {
    ...
    expect(resp.status).toBe(200);
    expect(resp.statusText).toBe("OK");
    expect(resp.data.data).toBe(
      JSON.stringify({ someName1: "someValue1", someName2: "someValue2" })
    );
  });
